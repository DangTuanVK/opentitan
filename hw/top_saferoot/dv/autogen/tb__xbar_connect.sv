...
`define DRIVE_CHIP_TL_HOST_IF(tl_name, inst_name, sig_name) \
     force ``tl_name``_tl_if.d2h = dut.top_saferoot.u_``inst_name``.``sig_name``_i; \
     force dut.top_saferoot.u_``inst_name``.``sig_name``_o = ``tl_name``_tl_if.h2d; \
     force dut.top_saferoot.u_``inst_name``.clk_i = 0; \
...
`define DRIVE_CHIP_TL_DEVICE_IF(tl_name, inst_name, sig_name) \
     force ``tl_name``_tl_if.h2d = dut.top_saferoot.u_``inst_name``.``sig_name``_i; \
     force dut.top_saferoot.u_``inst_name``.``sig_name``_o = ``tl_name``_tl_if.d2h; \
     force dut.top_saferoot.u_``inst_name``.clk_i = 0; \
     uvm_config_db#(virtual tl_if)::set(null, $sformatf("*env.%0s_agent", `"tl_name`"), "vif", \
                                        ``tl_name``_tl_if);
...
tl_if sram_ctrl__regs_tl_if(clk_main, rst_n);
tl_if sram_ctrl__ram_tl_if(clk_main, rst_n);
...
initial begin
  ...
  if (xbar_mode) begin
    ...
    $assertoff(0, tb);
    $asserton(0, tb.dut.top_saferoot.u_xbar_main);


    // These are all zero-time: anything that consumes time go at the end.

    // bypass clkmgr, force clocks directly
    force tb.dut.top_saferoot.u_xbar_main.clk_main_i = clk_main;
    force tb.dut.top_saferoot.u_xbar_main.clk_fixed_i = clk_io_div4;

    // bypass rstmgr, force resets directly
    force tb.dut.top_saferoot.u_xbar_main.rst_main_ni = rst_n;
    force tb.dut.top_saferoot.u_xbar_main.rst_fixed_ni = rst_n;
...
`ifndef GATE_LEVEL
    ...
    `DRIVE_CHIP_TL_DEVICE_IF(sram_ctrl__regs, sram_ctrl, regs_tl)
    `DRIVE_CHIP_TL_DEVICE_IF(sram_ctrl__ram, sram_ctrl, ram_tl)
