...
//############################################################################
// *Name: ast
// *Module Description: Analog Sensors Top  --  SafeRoot BLACK-BOX interface stub
//
// SafeRoot does not integrate an on-die AST: the analog companion (clock/reset/POK,
// entropy, sensing) is EXTERNAL, and there is no analog macro in the SAED32
// educational PDK (same black-boxing rationale as Flash/OTP/SRAM). This stub exists
// only so the ASIC chip-level wrapper `chip_saferoot_asic` is self-consistent and can
// be elaborated/linted. It is NOT part of the SafeRoot backend flow, whose synthesis
// target is `top_saferoot` and whose pad-ring is assembled in ICC2. Outputs are tied
// to safe defaults; do not use this model for functional/analog simulation.
//############################################################################
...
`endif
  ...
  // ---------------------------------------------------------------------------
  // BLACK-BOX tie-offs. External AST supplies these in silicon; here we present
  // safe constant defaults so the wrapper elaborates. Power-OK is asserted and
  // clocks are marked valid so reset/power sequencing in the wrapper releases.
  // ---------------------------------------------------------------------------
  import prim_mubi_pkg::MuBi4True;
  import prim_mubi_pkg::MuBi4False;

  assign tl_o             = '0;
  assign ast_init_done_o  = MuBi4True;

  assign ast_pwst_o       = '1;   // all power domains OK @1.1V
  assign ast_pwst_h_o     = '1;   // all power domains OK @3.3V

  assign flash_power_down_h_o  = 1'b0;
  assign flash_power_ready_h_o = 1'b1;
  assign otp_power_seq_h_o     = '0;

  assign clk_src_sys_o    = 1'b0;
  assign clk_src_sys_val_o = 1'b1;
  assign clk_src_aon_o    = 1'b0;
  assign clk_src_aon_val_o = 1'b1;
  assign clk_src_io_o     = 1'b0;
  assign clk_src_io_val_o = 1'b1;
  assign clk_src_io_48m_o = MuBi4False;
  assign clk_src_usb_o    = 1'b0;
  assign clk_src_usb_val_o = 1'b1;
  assign usb_io_pu_cal_o  = '0;

  assign adc_d_o          = '0;
  assign adc_d_val_o      = 1'b0;

  assign rng_val_o        = 1'b0;
  assign rng_b_o          = '0;
  assign entropy_req_o    = '0;

  assign alert_req_o      = '0;
  assign obs_ctrl_o       = '0;

  assign ast2padmux_o     = '0;
  assign mux_iob_sel_o    = '0;
  assign ast2pad_t0_ao    = 1'b0;
  assign ast2pad_t1_ao    = 1'b0;

  assign all_clk_byp_ack_o = MuBi4False;
  assign io_clk_byp_ack_o  = MuBi4False;
  assign flash_bist_en_o   = MuBi4False;

  assign dpram_rmf_o      = '0;
  assign dpram_rml_o      = '0;
  assign spram_rm_o       = '0;
  assign sprgf_rm_o       = '0;
  assign sprom_rm_o       = '0;

  assign dft_scan_md_o    = MuBi4False;
  assign scan_shift_en_o  = 1'b0;
  assign scan_reset_no    = 1'b1;

  // Suppress unused-input lint for the black-box interface.
  logic unused_inputs;
  assign unused_inputs = ^{
      tl_i, clk_ast_adc_i, rst_ast_adc_ni, clk_ast_alert_i, rst_ast_alert_ni,
      clk_ast_es_i, rst_ast_es_ni, clk_ast_rng_i, rst_ast_rng_ni,
      clk_ast_tlul_i, rst_ast_tlul_ni, clk_ast_usb_i, rst_ast_usb_ni,
      clk_ast_ext_i, por_ni, sns_clks_i, sns_rsts_i, sns_spi_ext_clk_i,
      vcc_supp_i, vcaon_supp_i, vcmain_supp_i, vioa_supp_i, viob_supp_i,
      main_pd_ni, main_env_iso_en_i, otp_power_seq_i, clk_src_sys_en_i,
      clk_src_sys_jen_i, clk_src_io_en_i, usb_ref_pulse_i, usb_ref_val_i,
      clk_src_usb_en_i, adc_pd_i, adc_a0_ai, adc_a1_ai, adc_chnsel_i,
      rng_en_i, rng_fips_i, entropy_rsp_i, alert_rsp_i, dft_strap_test_i,
      lc_dft_en_i, fla_obs_i, otp_obs_i, otm_obs_i, usb_obs_i, padmux2ast_i,
      ext_freq_is_96m_i, all_clk_byp_req_i, io_clk_byp_req_i};
