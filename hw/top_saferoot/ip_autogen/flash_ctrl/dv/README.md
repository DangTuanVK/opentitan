...
* [Simulation results](https://reports.opentitan.org/hw/top_saferoot/ip_autogen/flash_ctrl/dv/latest/report.html)
...
For the IP level DV, we replicate the instantiations and connections in `flash_ctrl_wrapper` module maintained in DV, located at `hw/top_saferoot/ip_autogen/flash_ctrl/dv/tb/flash_ctrl_wrapper.sv`.
...
### Top level testbench
Top level testbench is located at `hw/top_saferoot/ip_autogen/flash_ctrl/dv/tb/tb.sv`.
...
### Stimulus strategy
#### Test sequences
All test sequences reside in `hw/top_saferoot/ip_autogen/flash_ctrl/dv/env/seq_lib`.
...
Some of the most commonly used tasks / functions are as follows: From `hw/top_saferoot/ip_autogen/flash_ctrl/dv/env/seq/flash_ctrl_base_vseq.sv`,
* reset_flash
...
`hw/top_saferoot/ip_autogen/flash_ctrl/dv/env/flash_ctrl_env_cov.sv`
* control_cg
...
#### Assertions
* TLUL assertions: The `hw/top_saferoot/ip_autogen/flash_ctrl/dv/sva/flash_ctrl_bind.sv` binds the `tlul_assert` [assertions](../../../../ip/tlul/doc/TlulProtocolChecker.md) to the IP to ensure TileLink interface protocol compliance.
...
```console
$ cd $REPO_TOP
$ ./util/dvsim/dvsim.py hw/top_saferoot/ip_autogen/flash_ctrl/dv/flash_ctrl_sim_cfg.hjson -i flash_ctrl_smoke
```
