...
static const top_saferoot_plic_target_t kHart = kTopSaferootPlicTargetIbex0;
 ...
 * as the ISR.
 */
static volatile top_saferoot_plic_peripheral_t peripheral_expected;
...
void ottf_external_isr(uint32_t *exc_info) {
  ...
  top_saferoot_plic_peripheral_t peripheral = (top_saferoot_plic_peripheral_t)
      top_saferoot_plic_interrupt_for_peripheral[plic_irq_id];
  ...
  switch (peripheral) {
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralAlertHandler: {
      dif_alert_handler_irq_t irq =
          (dif_alert_handler_irq_t)(plic_irq_id -
                                    (dif_rv_plic_irq_id_t)
                                        kTopSaferootPlicIrqIdAlertHandlerClassa);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 1 && 1 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralAonTimerAon: {
      dif_aon_timer_irq_t irq =
          (dif_aon_timer_irq_t)(plic_irq_id -
                                (dif_rv_plic_irq_id_t)
                                    kTopSaferootPlicIrqIdAonTimerAonWkupTimerExpired);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralCsrng: {
      dif_csrng_irq_t irq =
          (dif_csrng_irq_t)(plic_irq_id -
                            (dif_rv_plic_irq_id_t)
                                kTopSaferootPlicIrqIdCsrngCsCmdReqDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralEdn0: {
      dif_edn_irq_t irq =
          (dif_edn_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopSaferootPlicIrqIdEdn0EdnCmdReqDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralEntropySrc: {
      dif_entropy_src_irq_t irq =
          (dif_entropy_src_irq_t)(plic_irq_id -
                                  (dif_rv_plic_irq_id_t)
                                      kTopSaferootPlicIrqIdEntropySrcEsEntropyValid);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralFlashCtrl: {
      dif_flash_ctrl_irq_t irq =
          (dif_flash_ctrl_irq_t)(plic_irq_id -
                                 (dif_rv_plic_irq_id_t)
                                     kTopSaferootPlicIrqIdFlashCtrlProgEmpty);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralGpio: {
      dif_gpio_irq_t irq =
          (dif_gpio_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopSaferootPlicIrqIdGpioGpio0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralHmac: {
      dif_hmac_irq_t irq =
          (dif_hmac_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopSaferootPlicIrqIdHmacHmacDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralI2c0: {
      dif_i2c_irq_t irq =
          (dif_i2c_irq_t)(plic_irq_id -
                          (dif_rv_plic_irq_id_t)
                              kTopSaferootPlicIrqIdI2c0FmtThreshold);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralKeymgr: {
      dif_keymgr_irq_t irq =
          (dif_keymgr_irq_t)(plic_irq_id -
                             (dif_rv_plic_irq_id_t)
                                 kTopSaferootPlicIrqIdKeymgrOpDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralKmac: {
      dif_kmac_irq_t irq =
          (dif_kmac_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopSaferootPlicIrqIdKmacKmacDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralOtbn: {
      dif_otbn_irq_t irq =
          (dif_otbn_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopSaferootPlicIrqIdOtbnDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 12 && 12 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralOtpCtrl: {
      dif_otp_ctrl_irq_t irq =
          (dif_otp_ctrl_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopSaferootPlicIrqIdOtpCtrlOtpOperationDone);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralPwrmgrAon: {
      dif_pwrmgr_irq_t irq =
          (dif_pwrmgr_irq_t)(plic_irq_id -
                             (dif_rv_plic_irq_id_t)
                                 kTopSaferootPlicIrqIdPwrmgrAonWakeup);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralRvTimer: {
      dif_rv_timer_irq_t irq =
          (dif_rv_timer_irq_t)(plic_irq_id -
                               (dif_rv_plic_irq_id_t)
                                   kTopSaferootPlicIrqIdRvTimerTimerExpiredHart0Timer0);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralSensorCtrl: {
      dif_sensor_ctrl_irq_t irq =
          (dif_sensor_ctrl_irq_t)(plic_irq_id -
                                  (dif_rv_plic_irq_id_t)
                                      kTopSaferootPlicIrqIdSensorCtrlIoStatusChange);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralSpiDevice: {
      dif_spi_device_irq_t irq =
          (dif_spi_device_irq_t)(plic_irq_id -
                                 (dif_rv_plic_irq_id_t)
                                     kTopSaferootPlicIrqIdSpiDeviceUploadCmdfifoNotEmpty);
...
#if TEST_MIN_IRQ_PERIPHERAL <= 17 && 17 < TEST_MAX_IRQ_PERIPHERAL
    case kTopSaferootPlicPeripheralUart0: {
      dif_uart_irq_t irq =
          (dif_uart_irq_t)(plic_irq_id -
                           (dif_rv_plic_irq_id_t)
                               kTopSaferootPlicIrqIdUart0TxWatermark);
...
static void peripheral_irqs_trigger(void) {
...
#if TEST_MIN_IRQ_PERIPHERAL <= 0 && 0 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralAlertHandler;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopSaferootPlicPeripheralAonTimerAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 2 && 2 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralCsrng;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 3 && 3 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralEdn0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 4 && 4 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralEntropySrc;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 5 && 5 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralFlashCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 6 && 6 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralGpio;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 7 && 7 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralHmac;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 8 && 8 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralI2c0;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 9 && 9 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralKeymgr;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 10 && 10 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralKmac;
  status_default_mask = 0x0;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 11 && 11 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralOtbn;
  ...
  if (kBootStage != kBootStageOwner) {
    peripheral_expected = kTopSaferootPlicPeripheralOtpCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 13 && 13 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralPwrmgrAon;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 14 && 14 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralRvTimer;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 15 && 15 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralSensorCtrl;
...
#if TEST_MIN_IRQ_PERIPHERAL <= 16 && 16 < TEST_MAX_IRQ_PERIPHERAL
  peripheral_expected = kTopSaferootPlicPeripheralSpiDevice;
  status_default_mask = 0x0;
  ...
  // non-DV setups.
  if (kDeviceType == kDeviceSimDV) {
    peripheral_expected = kTopSaferootPlicPeripheralUart0;
...
bool test_main(void) {
  ...
  rv_plic_testutils_irq_range_enable(
      &plic, kHart, kTopSaferootPlicIrqIdNone + 1, kTopSaferootPlicIrqIdLast);
